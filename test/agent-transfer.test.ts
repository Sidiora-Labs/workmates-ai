...
import {
  ...
  profileFromFile,
  type AgentFile,
} from "../server/agents/agent-transfer.ts";
...
describe("packing an agent", () => {
  ...
  test("a nameless agent still gets a name and a filename", () => {
    ...
    assert.equal(fileNameFor(file.agent.name), "agent.workmates-agent.json");
    assert.equal(fileNameFor("Head of Marketing"), "head-of-marketing.workmates-agent.json");
    assert.equal(fileNameFor("../../etc/passwd"), "etc-passwd.workmates-agent.json");
  });
