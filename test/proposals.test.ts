...
import {
  ...
  reviewPrompt,
  worthReviewing,
} from "../server/stores/proposals.ts";
import type { Turn } from "../server/agents/context.ts";
...
describe("staging, never installing", () => {
  const fresh = () => new ProposalStore(join(mkdtempSync(join(tmpdir(), "workmates-proposals-")), "proposals.json"));
  ...
  test("what is staged survives being reopened", () => {
    const file = join(mkdtempSync(join(tmpdir(), "workmates-proposals-")), "proposals.json");
