...
import { clickScript, focusScript, formatSnapshot, READ, SCAN } from "../server/integrations/page-script.ts";
...
describe("injected scripts", () => {
  test("a ref reaches the page as data, not as source", () => {
    ...
    assert.ok(!script.includes(`[data-workmates-ref="e1"] , [x"]`));
  });
  ...
  test("the scan bounds how much it returns", () => {
    ...
    assert.match(SCAN, /data-workmates-ref/);
  });
