...
function elicitation(id: number | string = 0, tool = "COMPOSIO_MULTI_EXECUTE_TOOL") {
  return {
    ...
    params: {
      threadId: "codex-thread", turnId: "codex-turn", serverName: "workmates_connectors",
      mode: "form", _meta: { codex_approval_kind: "mcp_tool_call" },
      message: `Allow the workmates_connectors MCP server to run tool "${tool}"?`,
...
test("Accept sends an MCP action with empty content, not a command decision", async (t) => {
  ...
  assert.equal(ask.tool, "mcp__workmates_connectors");
  ...
  const log = h.logs.find((l) => l.source === "workmates.approval" && l.msg.stage === "resolved").msg;
...
test("engine-cleared requests cannot be answered and do not send a second RPC reply", async (t) => {
  ...
  assert.ok(h.logs.some((l) => l.source === "workmates.approval" && l.msg.stage === "unavailable"));
});
