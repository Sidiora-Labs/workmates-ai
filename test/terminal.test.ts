...
import {
  ...
  spawnPlan,
  worthResizing,
} from "../server/stores/terminal.ts";
...
describe("getting a terminal out of the system", () => {
  ...
  test("the size is set inside the pty before the shell starts", () => {
    for (const platform of ["darwin", "linux"] as const) {
      ...
      assert.match(inner, /exec "\$WORKMATES_SHELL" -i/, "the shell replaces the setup rather than following it");
    }
  });
...
describe("a real session", { skip: hasPty ? false : "no pty on this platform" }, () => {
  test("it opens in the folder it was given and runs what you type", async () => {
    const dir = mkdtempSync(join(tmpdir(), "workmates-term-"));
    ...
    try {
      ...
      session.write("echo WORKMATES-$((6*7)) && ls\n");
      const got = await until(() => (/WORKMATES-42/.test(seen) ? true : null));
      assert.ok(got, `the command never ran. saw: ${JSON.stringify(seen.slice(0, 400))}`);
      const listed = await until(() => (/hello\.txt/.test(seen) ? true : null));
      assert.ok(listed, `it is not in the folder it was given. saw: ${JSON.stringify(seen.slice(0, 400))}`);
    } finally {
      detach();
      store.closeAll();
    }
  ...
  test("the same agent gets the same shell back, not a new one", async () => {
    ...
    const elsewhere = mkdtempSync(join(tmpdir(), "workmates-term-"));
