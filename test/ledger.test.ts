...
import { statementOf } from "../server/core/identity.ts";
import {
  GENESIS_PREV,
  Ledger,
  attribution,
...
describe("the file", () => {
  const fresh = () => new Ledger(join(mkdtempSync(join(tmpdir(), "workmates-ledger-")), "record.ndjson"));
  ...
  test("what was written is what a new process reads back", async () => {
    const dir = mkdtempSync(join(tmpdir(), "workmates-ledger-"));
  ...
  test("a record that cannot be written does not throw at the caller", async () => {
    const ledger = new Ledger("/nonexistent-directory-for-workmates/record.ndjson");
