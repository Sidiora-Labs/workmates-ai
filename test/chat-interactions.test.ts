...
test("room messages are visible and persisted while queued, then drain in order", async (t) => {
  ...
  t.after(() => c.stop());
  const path = `/api/rooms/${c.room.id}/messages`;
  ...
  const disk = JSON.parse(readFileSync(join(c.h.home, ".workmates", `messages-${c.room.id}.json`), "utf8"));
...
test("a queued room message taken back does not run", async (t) => {
  ...
  await c.h.fetch(`/api/threads/${c.room.id}/messages/${message.id}`, { method: "DELETE" });
...
test("room decision options send a single queued reply and persist the user's choice", async (t) => {
  const c = await chatHarness();
  t.after(() => c.stop());
  await c.post(`/api/rooms/${c.room.id}/messages`, { text: "@QueueAgent plan this" });
  await waitFor(() => c.calls.length === 1);
  ...
  const path = `/api/threads/${c.room.id}/messages/${card.id}/choose`;
...
test("solo decisions validate choices, preserve their task, and remain retryable on refusal", async (t) => {
  ...
  const saved = JSON.parse(readFileSync(join(c.h.home, ".workmates", `messages-${taskId}.json`), "utf8"));
