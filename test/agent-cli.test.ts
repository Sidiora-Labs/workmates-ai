...
import {
  ...
  matches,
  runsAProcess,
} from "../server/agents/agent-cli.ts";
...
describe("what an agent can do", () => {
  ...
  test("the things the job was posted for", () => {
    ...
    assert.equal(allows(ME, "POST", "/api/rooms").ok, true);
...
describe("the rooms an agent is in", () => {
  ...
  test("it can speak in a room it is in", () => {
    assert.equal(allows(ME, "POST", "/api/rooms/room-mine/messages", inRoom).ok, true);
    assert.equal(allows(ME, "PATCH", "/api/rooms/room-mine", inRoom).ok, true);
  });

  test("it cannot speak in a room it is not in", () => {
    const verdict = allows(ME, "POST", "/api/rooms/room-theirs/messages", inRoom);
  ...
  test("the refusal says it is about membership, not about the route", () => {
    const verdict = allows(ME, "PATCH", "/api/rooms/room-theirs", inRoom);
    assert.doesNotMatch(verdict.reason ?? "", /not on the list/);
  });

  test("with no rooms at all, nothing about a room is reachable", () => {
    assert.equal(allows(ME, "POST", "/api/rooms/anything/messages").ok, false);
    assert.equal(allows(ME, "GET", "/api/rooms").ok, true);
  });

  test("a room it is in is still not a room it may delete", () => {
    const verdict = allows(ME, "DELETE", "/api/rooms/room-mine", inRoom);
    assert.equal(verdict.ok, false);
  });
});
...
describe("the credential itself", () => {
  ...
  test("nothing and nonsense are nobody", () => {
    const tokens = new AgentTokens();
    for (const bad of [null, undefined, "", "wm_made-up", "Bearer x"]) {
...
describe("what the agent is told", () => {
  test("short, and it names the command", () => {
    const briefing = cliBriefing('node "/opt/workmates/bin/workmates.mjs"');
    assert.match(briefing, /\/opt\/workmates\/bin\/workmates\.mjs/);
