...
describe("keeping them", () => {
  const fresh = () => new PolicyStore(join(mkdtempSync(join(tmpdir(), "workmates-rules-")), "rules.json"));
  ...
  test("they survive being reopened", () => {
    const file = join(mkdtempSync(join(tmpdir(), "workmates-rules-")), "rules.json");
