...
before(async () => {
  stateDir = mkdtempSync(join(tmpdir(), "workmates-fake-rt-"));
  process.env.WORKMATES_SANDBOX_RUNTIME = resolve("test/helpers/fake-runtime.sh");
...
after(async () => {
  ...
  delete process.env.WORKMATES_SANDBOX_RUNTIME;
