...
test("a message for a room lands in the room, not an agent", () => {
  const state = withState({
    bots: [bot("a")],
    rooms: [{ id: "room-1", name: "Launch", memberIds: ["a"], createdAt: 0, messages: [] }],
  });
  const next = reducer(state, { type: "messageAdded", threadId: "room-1", message: msg("m1") });
  assert.equal(next.rooms[0].messages.length, 1);
...
test("answering a card shown in a room settles it in the room", () => {
  ...
  const state = withState({
    bots: [bot("a")],
    rooms: [{ id: "room-1", name: "R", memberIds: ["a"], createdAt: 0, messages: [card] }],
  });
  const next = reducer(state, {
    type: "answerCard",
    botId: "a",
  ...
  assert.equal(next.rooms[0].messages[0].card?.answered, "Yes");
