...
import {
  ...
  textOf,
  themeFrom,
} from "../server/integrations/mcp-apps.ts";
import { unwrapFrame } from "../server/integrations/mcp-client.ts";
...
describe("the terms it is framed under", () => {
  ...
  test("the app's colours go in, and nothing else about the app does", () => {
    ...
    assert.match(framed, /--workmates-bg:#101013/);
...
describe("what a framed app may ask for", () => {
  ...
  test("a link that is not the web is not a link", () => {
    for (const url of ["file:///etc/passwd", "javascript:alert(1)", "workmates://x", "//evil.example"]) {
