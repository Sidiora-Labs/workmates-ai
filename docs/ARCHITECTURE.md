...
1. `StoreProvider` loads `/api/bots`, `/api/rooms`, `/api/instances`,
...
4. The driver translates its provider's native output into the canonical
   events in `server/core/contracts.ts`.
...
A driver turns one provider into the contract in `server/core/contracts.ts`.
...
`server/integrations/providers.ts` rather than code. These are transcript-replay: the
...
Team formation (`server/agents/teams.ts`) sits on top: a senior agent can emit a
fenced plan, which becomes an approval card. Only your yes creates the
agents and the room.

## Persistence

Plain JSON under `~/.workmates`, written synchronously. No database.

| Path | Contents |
| --- | --- |
| `bots.json` | Agent records, model selection, resume cursors |
| `rooms.json` | Rooms and members |
...
- `server/core/http-guard.ts` checks `Origin` and `Host` on every request.
  Loopback is not a boundary in a browser.
- `server/core/limits.ts` holds every input cap in one place.
