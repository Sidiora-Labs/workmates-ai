...
const BITMAPS: Record<MateShape, { rows: string[]; faceY: number }> = {
  star: {
    rows: [
      ".......##.......",
      ".......##.......",
...
const BODIES: Record<MateShape, { d: string; faceY: number }> = Object.fromEntries(
...
) as Record<MateShape, { d: string; faceY: number }>;
...
function Face({ expression, fy }: { expression: MateExpression; fy: number }) {
  const stroke = {
    fill: "none",
    stroke: "#fff",
    strokeWidth: 3.4,
...
export function MateAvatar({
  ...
  color: MateColor;
  shape?: MateShape;
  expression?: MateExpression;
  size?: number;
  label?: string;
  className?: string;
}) {
  const uid = useId();
  const base = MATE_COLORS[color] ?? MATE_COLORS.green;
...
export function AgentAvatar({
  ...
  bot: { id: string; color: MateColor; avatarAt?: number | null } & Parameters<typeof shapeForBot>[0];
  size: number;
  className?: string;
}) {
  if (bot.avatarAt) {
