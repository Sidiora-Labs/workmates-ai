...
import { useMcpServers, type McpServerRow } from "../settings/McpServers";
import { AgentAvatar, MateAvatar } from "./Avatar";
import {
  expressionForBot,
  shapeForBot,
  MATE_COLORS,
...
import { VoiceCard } from "../chat/Voice";
...
export function SettingsPanel({ bot }: { bot: Bot }) {
  ...
  const shuffleLook = () =>
    patch({
      color: MATE_COLOR_NAMES[Math.floor(Math.random() * MATE_COLOR_NAMES.length)],
      shape: MATE_SHAPES[Math.floor(Math.random() * MATE_SHAPES.length)],
    });
            ...
            <div className="min-w-0 flex-1">
              ...
              {lookTab === "shape" && (
                <div className="grid grid-cols-6 gap-1">
                  {MATE_SHAPES.map((shape) => (
                    ...
                    >
                      <MateAvatar color={bot.color} shape={shape} expression="deadpan" size={30} />
                    </button>
                  ))}
                </div>
              )}
                ...
                <div className="grid grid-cols-5 gap-2 p-1">
                  {MATE_COLOR_NAMES.map((color) => (
                    <button
                      ...
                      style={{ backgroundColor: MATE_COLORS[color] }}
                      title={color}
              ...
              {lookTab === "face" && (
                <div className="grid grid-cols-5 gap-1">
                  {MATE_EXPRESSIONS.map((expression) => (
                    <button
                      key={expression}
...
function ApprovalsCard({ bot }: { bot: Bot }) {
  ...
  const choose = async (next: "ask" | "edits" | "auto") => {
    setRefused(false);
    if (next === "auto" && mode !== "auto" && window.rooms?.authConfirm) {
      const answer = await window.rooms.authConfirm(
...
function TakeItWithYouCard({ bot }: { bot: Bot }) {
  ...
  const save = async () => {
    ...
    try {
      ...
      a.href = url;
      a.download = named?.[1] ?? "agent.workmates-agent.json";
      a.click();
      URL.revokeObjectURL(url);
    } catch (e) {
      ...
      <div className="flex items-start justify-between gap-4">
        <div className="min-w-0">
          ...
          <div className="mt-0.5 text-[12.5px] leading-relaxed text-muted-foreground">
            ...
            look, the skills and the memory. Drop it into Workmates on another
