...
export function MessageActionBar({
  ...
  return (
    ...
    >
      {picking && onReact && (
        <div className="absolute bottom-full right-0 mb-1.5 flex gap-0.5 rounded-full bg-popover/90 p-1 shadow-[0_12px_32px_-8px_var(--shadow-color),0_0_0_0.5px_var(--border),0_0_0_1px_var(--shadow-color)] backdrop-blur-2xl backdrop-saturate-150">
  ...
  return (
    <div
      className={cn(
        ...
        onDark
          ? "border-brand-foreground/60 bg-brand-ink-foreground/20"
          : "border-brand/50 bg-foreground/[0.05]",
      )}
    >
      <span className="min-w-0 text-[11.5px] leading-snug">
        <span className={cn("font-medium", onDark ? "text-brand-foreground" : "text-brand-ink")}>
          {replyTo.author}
        </span>
        <span
          className={cn(
            "block truncate",
            onDark ? "text-brand-foreground/80" : "text-muted-foreground",
          )}
        >
          {replyTo.excerpt}
        </span>
...
export function ForwardDialog({
  ...
  const forward = (target: { id: string; kind: "agent" | "room" }) => {
    ...
    if (target.kind === "agent") {
      ...
      dispatch({ type: "sendToRoom", roomId: target.id, text: body });
    }
