...
import { AgentAvatar, MateAvatar } from "../agents/Avatar";
...
export function NewRoomDialog() {
    ...
    file
      ...
      .then(({ room }) =>
        api("/api/rooms").then(({ rooms }) => {
          dispatch({ type: "hydrateRooms", rooms });
  ...
  return (
    ...
    >
      <div
        className="flex max-h-[80%] w-[520px] max-w-[92vw] animate-pop-in flex-col rounded-[14px] bg-popover/90 p-4 shadow-[0_24px_64px_-12px_var(--shadow-color),0_0_0_0.5px_var(--border),0_0_0_1px_var(--shadow-color)] backdrop-blur-2xl backdrop-saturate-150 sm:p-5"
                    ...
                    <div className="mt-2.5 flex items-center">
                      {team.members.map((member, i) => (
                        <span
                          ...
                          title={member.title}
                        >
                          <MateAvatar color={member.color} shape={member.shape} size={22} />
                        </span>
                      ))}
