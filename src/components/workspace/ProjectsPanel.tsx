...
import { MATE_COLOR_NAMES, MATE_SHAPES, type MateColor, type MateShape } from "@/lib/mascot";
...
export function ProjectsPanel() {
  ...
  return (
    <div
      className="absolute inset-0 z-20 flex animate-fade-in items-center justify-center bg-black/30 dark:bg-black/55"
      onClick={close}
    >
      <div
        className="flex h-[76%] w-[600px] max-w-[92vw] animate-pop-in flex-col rounded-[14px] bg-popover/90 p-5 shadow-[0_24px_64px_-12px_var(--shadow-color),0_0_0_0.5px_var(--border),0_0_0_1px_var(--shadow-color)] backdrop-blur-2xl backdrop-saturate-150"
                ...
                return (
                  ...
                  >
                    <div className="flex items-start gap-3">
                      <MateAvatar color={project.color} shape={project.shape} expression="focused" size={30} />
...
function ProjectForm({
  ...
  const [color, setColor] = useState<MateColor>(project?.color ?? MATE_COLOR_NAMES[0]);
  const [shape, setShape] = useState<MateShape>(project?.shape ?? MATE_SHAPES[0]);
    ...
    <div className="flex flex-col gap-3">
      ...
      <div className="flex flex-wrap gap-1">
        ...
        {MATE_SHAPES.map((s) => (
          ...
          >
            <MateAvatar color={color} shape={s} expression="deadpan" size={18} />
          </button>
        ))}
      </div>
          ...
          {state.bots
            ...
            .map((bot) => {
              ...
              return (
                ...
                >
                  <MateAvatar color={bot.color} shape={bot.shape} expression="friendly" size={18} />
                  {bot.name}
                </button>
              );
            })}
