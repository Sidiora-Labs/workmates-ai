...
import { api, useStore, formatWhen, type Room, type Bot } from "@/state/store";
import { Button } from "@/components/ui/button";
import { useTheme } from "@/lib/theme";
import { AgentAvatar } from "../agents/Avatar";
import { WorkmatesLogo, WorkmatesMark } from "../welcome/Brand";
...
function SectionPicker({ filing, onClose }: { filing: FilingState; onClose: () => void }) {
  ...
  const fileTo = (section: string | null) => {
    if (filing.kind === "bot") {
      ...
      dispatch({ type: "patchRoom", roomId: filing.id, patch: { section } });
    }
    onClose();
  };
  ...
  return (
    ...
    >
      <div className="w-[300px] animate-pop-in rounded-[14px] bg-popover/90 p-4 shadow-[0_24px_64px_-12px_var(--shadow-color),0_0_0_0.5px_var(--border),0_0_0_1px_var(--shadow-color)] backdrop-blur-2xl backdrop-saturate-150">
...
function BotContextMenu({
  ...
  const item = (
    ...
    <button
      ...
      className={cn(
        "flex w-full items-center gap-2.5 rounded-[6px] px-2.5 py-[5px] text-left text-[13px]",
        opts?.danger
          ? "text-destructive hover:bg-destructive hover:text-destructive-foreground hover:[&_svg]:text-destructive-foreground"
          : "text-foreground hover:bg-brand-ink hover:text-brand-foreground hover:[&_svg]:text-brand-foreground",
      )}
    >
      {icon}
      {label}
  ...
  const divider = (key: string) => <div key={key} className="mx-2 my-[5px] h-px bg-border" />;

  return (
    <div
      data-bot-menu
      style={{ top, left }}
      className="fixed z-40 w-[208px] animate-pop-in rounded-[10px] bg-popover/85 p-[5px] shadow-[0_12px_32px_-8px_var(--shadow-color),0_0_0_0.5px_var(--border),0_0_0_1px_var(--shadow-color)] backdrop-blur-2xl backdrop-saturate-150"
    >
      {[
        item(
          bot.pinned ? (
...
function BotListItem({
  ...
  if (rail) {
    return (
      <button
        ...
        title={bot.name}
        className={cn(
          "relative flex shrink-0 self-center rounded-[10px] p-1.5 transition-[background-color,transform] duration-150 active:scale-95",
          selected ? "bg-brand-ink" : "hover:bg-accent/60",
        )}
      >
  ...
  return (
    <button
      ...
      className={cn(
        "flex w-full items-center gap-3 rounded-[10px] px-2.5 py-2 text-left transition-[background-color,transform] duration-150 active:scale-[0.99]",
        selected
          ? "bg-brand-ink text-brand-foreground [&_span]:text-brand-foreground [&_svg.lucide]:text-brand-foreground [&_.dot]:bg-brand-foreground"
          : "hover:bg-accent/60",
      )}
    >
      ...
      <div className="min-w-0 flex-1">
        ...
        <div className="mt-px flex items-center justify-between gap-2">
          ...
          {bot.unread && <span className="dot size-2 shrink-0 rounded-full bg-brand" />}
        </div>
      </div>
    </button>
  );
  ...
  if (rail) {
    return (
      <button
        ...
        title={room.name}
        className={cn(
          "flex shrink-0 self-center rounded-[10px] p-1.5 transition-[background-color,transform] duration-150 active:scale-95",
          selected ? "bg-brand-ink" : "hover:bg-accent/60",
        )}
      >
  ...
  return (
    <button
      ...
      onContextMenu={(e) => {
        if (!onFile) return;
        e.preventDefault();
        onFile({ kind: "room", id: room.id, name: room.name, current: room.section ?? null });
      }}
      className={cn(
        "flex w-full items-center gap-3 rounded-[10px] px-2.5 py-2 text-left transition-[background-color,transform] duration-150 active:scale-[0.99]",
        selected
...
export function Sidebar() {
  ...
  const [choice, setChoice] = useState<boolean | null>(() => {
    const saved = localStorage.getItem("workmates-sidebar");
  ...
  const toggleCollapsed = () => {
    const next = !rail;
    setChoice(next);
    localStorage.setItem("workmates-sidebar", next ? "rail" : "full");
  };
  ...
  if (mobile) {
    return (
      <>
        <header className="sidebar-surface flex h-[54px] w-full shrink-0 items-center gap-1 border-b pl-3 pr-1.5">
          <WorkmatesMark size={17} />
          <DropdownMenu>
          ...
          <div className="flex min-w-0 flex-1 items-center gap-0.5 overflow-x-auto px-0.5">
            {state.rooms.map((b) => (
              <RoomListItem key={b.id} room={b} rail />
            ))}
            {visibleBots.map((b) => (
  ...
  return (
    <aside
      className={cn(
        "sidebar-surface flex h-full shrink-0 flex-col border-r transition-[width] duration-200",
        rail ? "w-[96px]" : "w-[320px]",
      )}
    >
      {isElectron && (
      ...
      {rail ? (
        ...
        <div
          ...
          style={{ WebkitAppRegion: "drag" } as React.CSSProperties}
        >
          <WorkmatesLogo className="h-[20px]" />
          <div
      ...
      {!rail && (
        <div className="px-3 pb-2 pt-1">
          <div className="flex items-center gap-2 rounded-[9px] border border-input/70 bg-background/70 px-2.5 py-[6px] shadow-[inset_0_1px_1px_var(--shadow-color)] transition-[border-color,box-shadow] duration-150 focus-within:border-ring focus-within:shadow-[0_0_0_3px_color-mix(in_srgb,var(--ring)_28%,transparent)]">
      ...
      <div className="flex-1 overflow-y-auto px-2 pt-1">
        <div className={cn("flex flex-col", rail ? "gap-1" : "gap-px")}>
          {(rail ? state.rooms : inSection(state.rooms, null)).length > 0 && (
            <>
              {!rail && (
                <div className="flex items-center justify-between px-2.5 pb-1 pt-1 text-[11px] font-semibold text-muted-foreground">
                  Rooms
                  <button
              ...
              {(rail ? state.rooms : inSection(state.rooms, null)).map((b) => (
                <RoomListItem key={b.id} room={b} rail={rail} onFile={setFiling} />
              ))}
              {rail ? (
                <div className="mx-3 my-1 border-t" />
              ) : (
                <div className="px-2.5 pb-1 pt-3 text-[11px] font-semibold text-muted-foreground">
                  Agents
                </div>
              )}
            </>
          ...
          {!rail &&
            sectionNames(visibleBots, state.rooms).map((name) => {
              const rooms = inSection(state.rooms, name);
              const bots = inSection(visibleBots, name);
              if (!rooms.length && !bots.length) return null;
              return (
                <div key={name} className="flex flex-col gap-px">
                  <div className="flex items-center gap-1.5 px-2.5 pb-1 pt-3 text-[11px] font-semibold text-muted-foreground">
                    <Folder size={11} className="shrink-0" />
                    <span className="truncate">{name}</span>
                  </div>
                  {rooms.map((b) => (
                    <RoomListItem key={b.id} room={b} onFile={setFiling} />
                  ))}
                  {bots.map((b) => (
...
function ArchivedAgents({ onClose }: { onClose: () => void }) {
  ...
  return (
    <div
      className="fixed inset-0 z-40 flex animate-fade-in items-center justify-center bg-black/30 dark:bg-black/55"
      onClick={onClose}
    >
      <div
        className="w-[420px] max-w-[92vw] animate-pop-in rounded-[14px] bg-popover/90 p-4 shadow-[0_24px_64px_-12px_var(--shadow-color),0_0_0_0.5px_var(--border),0_0_0_1px_var(--shadow-color)] backdrop-blur-2xl backdrop-saturate-150 sm:p-5"
