...
export function CommandPalette() {
  ...
  const rooms = useMemo(
    () => rankByName(state.rooms, (r) => r.name, query.trim()),
    [state.rooms, query],
  );
  ...
  const activate = (index: number) => {
    if (index < bots.length) {
      ...
      if (!hit) return;
      if (hit.roomId) {
        dispatch({ type: "select", id: hit.roomId });
      } else if (hit.botId) {
  ...
  return (
    ...
    >
      <div
        className="flex max-h-[60vh] w-[560px] max-w-[92vw] animate-pop-in flex-col overflow-hidden rounded-[14px] bg-popover/90 shadow-[0_24px_64px_-12px_var(--shadow-color),0_0_0_0.5px_var(--border),0_0_0_1px_var(--shadow-color)] backdrop-blur-2xl backdrop-saturate-150"
