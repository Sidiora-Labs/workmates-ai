  ...
  return (
    <div className="flex flex-1 flex-col items-center justify-center gap-4 px-8 text-center">
      <div className="flex items-center gap-2">
        <MateAvatar color="blue" shape="star" expression="friendly" size={52} />
        <MateAvatar color="green" shape="cloud" expression="excited" size={52} />
      </div>
      <div>
...
function WebhooksTab({
  ...
  const target = selected ? targetOf(selected.botId ?? selected.roomId ?? "") : null;
  ...
  const create = (targetId: string) => {
    ...
    const body = bot ? { botId: targetId } : { roomId: targetId };
    api("/api/webhooks", {
      method: "POST",
    ...
    <div className="flex min-h-0 flex-1 flex-col md:flex-row">
      <div className="flex max-h-[180px] w-full shrink-0 flex-col border-b md:max-h-none md:w-[260px] md:border-b-0 md:border-r">
        ...
        <div className="min-h-0 flex-1 overflow-y-auto p-2">
          {(hooks ?? []).map((hook) => {
            const t = targetOf(hook.botId ?? hook.roomId ?? "");
            return (
              <button
                key={hook.id}
