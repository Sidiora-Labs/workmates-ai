...
function pairLink(address: string, port: number, token: string, code: string): string | null {
  ...
  if (!/^workmates_pair_[A-Za-z0-9_-]{32}$/.test(token)) return null;
  ...
  return `workmates://pair?address=${encodeURIComponent(`${host}:${port}`)}&token=${token}&code=${code}&name=${name}`;
}
      ...
      <div className="flex items-center justify-between gap-3">
        <div>
          ...
          <div className="mt-0.5 text-[12.5px] leading-relaxed text-muted-foreground">
            Lets the Workmates app on your phone reach this Mac over your network.
          </div>
        </div>
          ...
          {window_ && secondsLeft > 0 ? (
            <div className="flex items-start gap-4">
              <div className="shrink-0 rounded-xl bg-white p-2 shadow-sm" aria-label="Pairing QR code">
                {window_.qr ? (
                  <img src={window_.qr} alt="Scan with the Workmates app" className="size-[152px]" />
                ) : (
