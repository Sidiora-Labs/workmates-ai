...
import { BlockField, WorkmatesLogo } from "./Brand";
...
export function Onboarding({ onDone }: { onDone: () => void }) {
  ...
  const startFresh = () => {
    ...
    void fetch("/api/workspace/reset", { method: "POST" })
      .then(() => {
        localStorage.removeItem("workmates-setup-done");
        location.reload();
      })
  ...
  useEffect(() => {
    ...
    if (step === 3 && isElectron) {
      const poll = () => window.rooms?.permStatus?.().then(setPerms).catch(() => {});
      poll();
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-background p-4">
      <BlockField />
      <div className="relative flex w-full max-w-[440px] animate-pop-in flex-col rounded-[14px] bg-popover/90 p-6 shadow-[0_24px_64px_-12px_var(--shadow-color),0_0_0_0.5px_var(--border),0_0_0_1px_var(--shadow-color)] backdrop-blur-2xl backdrop-saturate-150 sm:p-8">
          ...
          <div className="flex flex-col">
            ...
            <div className="mt-4 flex flex-col gap-2">
              <PermissionRow
                ...
                status={perms?.mic}
                onEnable={() =>
                  window.rooms?.permRequestMic?.().then(() => window.rooms?.permStatus?.().then(setPerms))
                }
